...
|---|---|---|---|---|
| `/grill-with-docs`, `/grill-me` (or `/team-grill` standalone) | Product Owner | `lgpd-analyst`, `security-analyst`, `qa-expert`; `ux-expert` if UI; `dba` if persistence; `sre` if it will run in production | dispatched before each round with the settled decisions and the open frontier | facts into the design tree; candidate questions into the round, tagged by origin |
| `/to-spec` | Product Owner | every active advisor | the draft spec | positions; maintained objections quoted in the spec |
| `/to-tickets` | Tech Lead | `senior-developer` | the ticket set | implementability findings and missing blocking edges |
| `/implement` (or `/team-implement` standalone) | Tech Lead | `senior-developer` x N (one per ticket, worktree); `qa-expert` on every returned branch; `ux-expert` if UI, `dba` if schema or migration, `security-analyst` if auth or personal data, `sre` if deploy, CI, logging, scheduling or an outbound call | tickets whose blockers are closed | review verdicts; the merge; escalation after the loop limit |
...
- **lgpd-analyst**: personal data under LGPD (Lei 13.709/2018): legal basis, sensitive data, minimisation, retention, data subject rights, controller and processor roles, international transfer. Finds where the repo already stores personal data before asking about it.
